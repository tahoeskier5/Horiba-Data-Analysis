...
OUTPUT_DIR = r"D:\Desktop\Corrosion Test Bench Data\Test 2 June 2025\Temperature Sensor Change\PT4 Temperature\Baseline"
...
def ror_global_flow_calc(df: pd.DataFrame):
    ...
    t_kelvin = df["PT4 Temperature"].mean() + ABS_ZERO
    slope, _, r_value, _, _ = linregress(df["minute"], df["PT4 Pressure"])
...
def ror_iterative_flow_calc(df: pd.DataFrame) -> pd.DataFrame:
    ...
    t_kelvin = df["PT4 Temperature"] + ABS_ZERO
    time = df["minute"].values
    pressure = df["PT4 Pressure"].values
