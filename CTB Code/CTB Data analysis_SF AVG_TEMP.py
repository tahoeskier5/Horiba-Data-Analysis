...
CRAWL_DIR = r"D:\Desktop\Corrosion Test Bench Data\Test 2 June 2025\N2 Baseline\2025.06.04 N2 Baseline"
OUTPUT_DIR = r"D:\Desktop\Corrosion Test Bench Data\Test 2 June 2025\Temperature Sensor Change\Average Temperature\Baseline"
...
def add_average_temperature(df: pd.DataFrame) -> pd.DataFrame:
    """
    Adds a new column 'Average Temperature' to df, calculated as the mean of:
      - 'PT3 Temperature'
      - 'PT4 Temperature'
      - first column that contains 'Temperature Reading' in its name (excluding PT3/PT4)
    """
    temp_reading_cols = [c for c in df.columns 
                         if "Temperature Reading" in c 
                         and c not in ("PT3 Temperature", "PT4 Temperature")]
    
    if not temp_reading_cols:
        raise KeyError("No column found containing 'Temperature Reading' besides PT3/PT4 Temperature.")
    
    reading_col = temp_reading_cols[0]
    df["Average Temperature"] = df[["PT3 Temperature", "PT4 Temperature", reading_col]].mean(axis=1)
    return df
...
def ror_global_flow_calc(df: pd.DataFrame):
    """Compute global ROR flow and R² via linear regression using average temperature."""
    if "Average Temperature" not in df.columns:
        df = add_average_temperature(df)
    t_kelvin = df["Average Temperature"].mean() + ABS_ZERO
...
def ror_iterative_flow_calc(df: pd.DataFrame) -> pd.DataFrame:
    """Compute per-row slope and flow using central differences and average temperature."""
    if "Average Temperature" not in df.columns:
        df = add_average_temperature(df)
    t_kelvin = df["Average Temperature"] + ABS_ZERO
...
def ror_analysis(df: pd.DataFrame, output_dir: str, file_stem: str):
    ...
    temp_cols = [c for c in df.columns
                 if re.search(r"Temperature\s+Reading\s*\[REAL\]$", c)]
    ...
    for sp in setpoints:
        ...
        results.append({
            ...
            "ave_ror_temp":  subset["PT4 Temperature"].mean(),
...
def crawl_dir(folder_path: str,
    ...
    for file in os.listdir(folder_path):
        ...
        # ——— Special case: only first 7 cycles for DUT6 run27 ———
        if file_stem == "2025-06-06_07-48_ROR-DUT6-RUN27":
            print(f"[{file_stem}] Callout: restricting to first 7 cycles")
            allowed_cycles = sorted(df["Cycle"].unique())[:7]
            df = df[df["Cycle"].isin(allowed_cycles)]
