...
# Usage
folder = r'D:\Desktop\Corrosion Test Bench Data\Test 2 June 2025\N2 Baseline\2025.06.04 N2 Baseline\Results'
...
def compute_ror_stats(concat_df, mean_time):
    ...
    _ = concat_df.groupby(
        ['Cycle', 'DUT', 'ROR Setpoint','Test Run'], as_index=False
    )['ror_flow_iterative'].describe()

    # 2) Perform custom aggregation on ['count','mean','min','max','median','std']
    stats_df = (
        concat_df
        .groupby(['Cycle', 'DUT', 'ROR Setpoint','Test Run'], as_index=False)
    ...
    keep_mean_cols = ['DUT', 'ROR Setpoint', 'Cycle','Test Run'] + [
    ...
    results_df = (
        stats_mean
        .groupby(['DUT', 'ROR Setpoint','Test Run'], as_index=False)
    ...
    results_df_ror = results_df[['DUT', 'ROR Setpoint','Test Run'] + ror_cols].copy()
...
def plot_ror_iterative(results_df_ror):
    ...
    results_df_ror : pandas.DataFrame
        Must contain columns:
          - 'DUT'
          - 'ROR Setpoint'
          - 'ROR Iterative Repeatability[%RDG]'
    """
    # Ensure columns exist
    required_cols = {'DUT', 'ROR Setpoint', 'ROR Iterative Repeatability [%RDG]'}
    if not required_cols.issubset(results_df_ror.columns):
        raise KeyError(f"DataFrame must contain columns: {required_cols}")

    # Get sorted unique DUT values
    unique_duts = sorted(results_df_ror['DUT'].unique())
    n_duts = len(unique_duts)

    # Qualitative colormap with enough distinct colors
    cmap = plt.cm.get_cmap('tab20', n_duts)

    # Define a list of linestyles and marker styles to cycle through
    linestyles = ['-', '--', '-.', ':']
    markers = ['o', 's', '^', 'D', 'v', 'P', 'X', '*']
    ...
    for idx, dut in enumerate(unique_duts):
        subset = results_df_ror[results_df_ror['DUT'] == dut].copy()
        subset.sort_values('ROR Setpoint', inplace=True)

        x = subset['ROR Setpoint']
        y = subset['ROR Iterative Repeatability [%RDG]']
        color = cmap(idx)
        linestyle = linestyles[idx % len(linestyles)]
        marker = markers[idx % len(markers)]

        # Plot line with specified linestyle
        plt.plot(
            x, y,
            color=color,
            linestyle=linestyle,
        ...
        # Plot scatter with specified marker
        plt.scatter(
            x, y,
            label=f"DUT {int(dut)}",
            color=color,
    ...
    # Labels, title, legend with larger fonts
    plt.xlabel("ROR Setpoint", fontsize=14)
    plt.ylabel("ROR Iterative 2-σ Repeatability [%RDG]", fontsize=14)
    plt.title("N2 Baseline ROR Repeatability", fontsize=16)

    legend = plt.legend(title="DUT", title_fontsize=12, fontsize=12)
