...
folder_path = r"\\shr-eng-srv\Share\Corrosion Test Bench\CTB Data\ELF19\20250120-N2_Test_Post_HF_Soak3"
filename = "D700 ROR Test.csv"

save_path = r'\\shr-eng-srv\Share\Corrosion Test Bench\CTB Data\ELF19\20250120-N2_Test_Post_HF_Soak3\Results decimated data'
...
df = select_columns(data, {'P0 (PSI)','P1 (Torr)','P2 (Torr)','Flow (Sccm)','Setpoint Readback','Temperature'},{'Time','COM4-3:MicroRoc-0x01-1:P1 Mixed Torr-17'})
...
def combine_dataframes_with_device_names(dataframes_dict):
    ...
    for device_name, df in dataframes_dict.items():
        ...
        if microroc_df is not None:
            df['MicroRoc Time'] = microroc_df['Time']
...
combined_df = add_scc_column(combined_df, 'MicroRoc', 'Temperature (C)-4')
...
combined_df = combined_df[
    ...
    (combined_df['P2 (Torr)-3'] > 50) & 
    (combined_df['P2 (Torr)-3'] < 400)
]
...
combined_df_filt = filter_by_iqr_and_zscore(combined_df, 'sccm', ['Device Name','Setpoint Readback-10','Cycle'])
...
combined_group = filtered_combined_df.groupby(['Device Name', 'Setpoint Readback-10', 'Cycle'])
...
mean_columns = ['Device Name', 'Setpoint Readback-10', 'Cycle'] + [col for col in stats_df.columns if '_mean' in col]
stats_mean = stats_df[mean_columns]

# Group by 'Device Name' and 'Setpoint Readback-10'
stats_mean_group = stats_mean.groupby(['Device Name', 'Setpoint Readback-10'])
...
# save_result_to_csv(results_df_sccm, save_path, filename, suffix = "_results_skip_4")
# save_result_to_csv(stats_df, save_path, filename, suffix='_stats_skip_4')
